...
#pragma multi_compile __ DOF_FRONTBLUR
	...
	uniform float DofSensorSize;// = 2.0;
		...
		half3 LensBlurMobile(Texture2D inTex, SamplerState Tex, float2 texCoord, float max_radius, float2 texelSize) {
			half3 outColor = inTex.SampleLevel(Tex, texCoord, 0.0).xyz;
			half weight = 1.0;
			...
			for (int i = 1; i <= DoFRings; i++) {
				...
				for (int j = 0; j < ringsamples; j++) {
					...
					outColor += inTex.SampleLevel(Tex,
						texCoord + half2(pw*w, ph*h), 0.0).rgb
						*lerp(1.0, (half(i)) / ((half)DoFRings), 0.3);
					//outColor += UNITY_SAMPLE_TEX2D(_MainTex, texCoord + float2(pw*w, ph*h)).rgb*lerp(1.0, (float(i)) / ((float)DoFRings), 0.3);
		...
		half4 DoFBokehTresholdMobilePass(float2 uv)
		{
			...
			half3 edgeBlur = LensBlurMobile(_MainTex, sampler_MainTex, uv, Blur*DoFMaxBlur, _MainTex_TexelSize.xy);
		...
		float4 DoFPass(float2 uv)
		{
			...
			float3 blurCol = //(float3)0.0;//
							LensBlur(_MainTex, sampler_MainTex, uv, Blur*DoFMaxBlur, _MainTex_TexelSize.xy);
			float4 edgeBlur = UNITY_SAMPLE_TEX2D(_SecondaryTex, uv);//(float3)0.0;
							//LensBlur(_SecondaryTex, sampler_SecondaryTex, uv, FrontBlur*DoFMaxBlur, _MainTex_TexelSize.xy);
							//N_EdgeShapedBlur(_SecondaryTex, sampler_SecondaryTex, uv, BokehDoFEdgeCount, BokehDoFSamplePerEdge, frontBlur*DoFMaxBlur, _MainTex_TexelSize.xy)*BokehDoFIntensity;
